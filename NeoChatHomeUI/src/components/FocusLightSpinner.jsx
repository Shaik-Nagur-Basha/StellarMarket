...
export default function FocusLightSpinner({
  ...
  // Updated media path construction
  const contentMetadata = contentArray.map((content) => ({
    ...
    media:
      content.type === "image"
        ? `${window.location.origin}/images/${content.title}.png`
        : `${window.location.origin}/Videos/${content.title}.mp4`,
  }));
  ...
  const handleMediaLoad = (itemId) => {
    console.log(`Media loaded successfully for ${itemId}`);
    setTimeout(() => {
  ...
  const handleMediaError = (itemId) => {
    const mediaItem = contentMetadata.find((item) => item.title === itemId);
    console.error(`Media load error for ${itemId}:`, {
      path: mediaItem?.media,
      type: mediaItem?.type,
    });
    setLoadingStates((prev) => ({
      ...prev,
                ...
                >
                  ...
                  >
                    <div className={styles.mediaContainer}>
                      <div
                        className={`${styles.cardMedia} relative group ${
