...
function RealTimeUserActivityDashboard() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      activityChart.data.datasets[0].data.push(140 + Math.floor(Math.random() * 30));
      // Keep last 6 entries
  ...
  return (
    <div className="min-h-screen bg-gray-100 pt-20 pb-10 px-4">
      <div className="max-w-3xl mx-auto bg-white rounded-lg shadow-xl p-6">
        <h1 className="text-3xl font-bold mb-6 text-center">Real-Time User Activity Dashboard</h1>
        <canvas ref={chartRef} />
      </div>
    </div>
  );
...
export default RealTimeUserActivityDashboard;
